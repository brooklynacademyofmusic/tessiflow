...
test_that("error_handler sends email when not interactive and configured", {
  ...
  expect_output(
    expect_warning(tryCatch(eval(error),error=error_handler),
...
test_that("error_email emails the error",{
  stub(send.mail,".jTryCatch",function(...){rlang::warn(class="sent!")})
...
test_that("error_email reads flow_name and job_name",{
  ...
  expect_match(mock_args(send.mail)[[1]][["subject"]],"something.+something else")
  expect_match(mock_args(send.mail)[[1]][["body"]],"something.+something else")
})
...
test_that("error_email reads flow_name and job_name from error_handler_factory",{
  ...
  stub(error_handler_factory,"job_on_error",function(flow_name,job_name,error){error_email(error)})
  
  f <- error_handler_factory(function(flow_name,job_name) {stop("I am an error")})
