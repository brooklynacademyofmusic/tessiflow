...
flows_report_load <- function() {
  ...
  tasks <- tbl(tessiflow$db,"jobs") %>% filter(start_time > !!as.integer(now() - ddays(1))) %>%
    collect %>%
    transmute(`Flow Name`=flow_name,
              `Job Name`=job_name,
...
flows_report_send <- function() {
    ...
    body <- flows_report_load() %>% 
      addHtmlTableStyle(col.rgroup=case_when(.$Status=="Failed"~"#F77",
                                             .$Status=="Running"~"#7F7",
                                             TRUE~"none")) %>% 
